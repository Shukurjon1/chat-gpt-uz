 ...
 */
function escapeForMarkdown(input) {
...
function formatVariables(input, optionalParameters) {
    return input.replace('$personality', userConfig.personality || PARAMETERS.PERSONALITY)
        ...
        .replace('$username', (optionalParameters === null || optionalParameters === void 0 ? void 0 : optionalParameters.username) || 'username')
...
const PARAMETERS = {
    ...
    CONTINUOUS_CONVERSATION: process.env.CONTINUOUS_CONVERSATION ?
        JSON.parse(process.env.CONTINUOUS_CONVERSATION) : true,
...
bot.setMyCommands([
    {
        command: 'start',
        description: TRANSLATIONS[userConfig.language || PARAMETERS.LANGUAGE]['command-descriptions'].start
    },
    {
        command: 'personality',
        description: TRANSLATIONS[userConfig.language || PARAMETERS.LANGUAGE]['command-descriptions'].personality
    },
    {
        command: 'name',
        description: TRANSLATIONS[userConfig.language || PARAMETERS.LANGUAGE]['command-descriptions'].name
    },
    {
        command: 'imagine',
        description: TRANSLATIONS[userConfig.language || PARAMETERS.LANGUAGE]['command-descriptions'].imagine
    },
    {
        command: 'reset',
        description: TRANSLATIONS[userConfig.language || PARAMETERS.LANGUAGE]['command-descriptions'].reset
    },
    {
        command: 'language',
        description: TRANSLATIONS[userConfig.language || PARAMETERS.LANGUAGE]['command-descriptions'].language
    },
]);
...
bot.on('message', async (msg) => {
    ...
    for (const command of (await bot.getMyCommands())) {
        if ((_a = msg.text) === null || _a === void 0 ? void 0 : _a.startsWith('/' + command.command))
            return;
    }
    if (msg.text && (msg.chat.type == 'private' || ((_b = msg.text) === null || _b === void 0 ? void 0 : _b.includes(`@${botUsername}`)))) {
        const text = (_c = msg.text) === null || _c === void 0 ? void 0 : _c.replace('@' + botUsername + ' ', '').replace('@' + botUsername, '').replace('#', '\\#');
        const username = ((_d = msg.from) === null || _d === void 0 ? void 0 : _d.username) || 'user';
        ...
        try {
            ...
            console.log(`\n${suffix}: "${text}"\n${botName}: "${response}"`);
            console.log(`[usage: ${((_e = ai.data.usage) === null || _e === void 0 ? void 0 : _e.total_tokens) || -1} tokens ` +
                `($${(((_f = ai.data.usage) === null || _f === void 0 ? void 0 : _f.total_tokens) || 0) * price})]`);
            if (PARAMETERS.CONTINUOUS_CONVERSATION) {
                lastMessage += buildLastMessage(suffix, text, response) + '\n';
                fs.appendFileSync('history.jsonl', JSON.stringify({
                    prompt: `${suffix}: ###${text}###\n${botName}: ###`,
                    completion: response
                }) + '\n');
            }
            ...
            await bot.sendMessage(msg.chat.id, TRANSLATIONS[userConfig.language || PARAMETERS.LANGUAGE].errors['generic-error'], { reply_to_message_id: msg.message_id });
            console.error(e);
            return;
        }
    }
...
bot.onText(/^\/(\w+)(@\w+)?(?:\s.\*)?/, async (msg, match) => {
    ...
    if (match.input.split(' ').length != 1) {
        ...
        if (!(command.startsWith('/reset') || command.startsWith('/start'))) {
            await bot.sendMessage(msg.chat.id, formatVariables(TRANSLATIONS[userConfig.language || PARAMETERS.LANGUAGE]
                .errors['no-parameter-command'], { command }), { reply_to_message_id: msg.message_id });
            return;
        }
    }
    if (command === null || command === void 0 ? void 0 : command.endsWith('@' + botUsername)) {
    ...
    switch (command) {
        case '/start':
            await bot.sendMessage(msg.chat.id, formatVariables(TRANSLATIONS[userConfig.language || PARAMETERS.LANGUAGE]
                .general['start-message']), { reply_to_message_id: msg.message_id });
            break;
        case '/personality':
            switchPersonality(input);
            await bot.sendMessage(msg.chat.id, formatVariables(TRANSLATIONS[userConfig.language || PARAMETERS.LANGUAGE]
                .general['personality-switch']), { reply_to_message_id: msg.message_id });
            break;
        case '/name':
            switchBotName(input);
            await bot.sendMessage(msg.chat.id, formatVariables(TRANSLATIONS[userConfig.language || PARAMETERS.LANGUAGE].general['name-switch']), { reply_to_message_id: msg.message_id });
            break;
        case '/reset':
            resetBotMemory();
            await bot.sendMessage(msg.chat.id, TRANSLATIONS[userConfig.language || PARAMETERS.LANGUAGE].general['memory-reset'], { reply_to_message_id: msg.message_id });
            break;
        case '/language':
            if (Object.keys(TRANSLATIONS).includes(input)) {
                switchLanguage(input);
                await bot.sendMessage(msg.chat.id, TRANSLATIONS[input].general['language-switch'], { reply_to_message_id: msg.message_id });
                break;
            }
            await bot.sendMessage(msg.chat.id, TRANSLATIONS[userConfig.language || PARAMETERS.LANGUAGE]
                .errors['invalid-language'].replace('$language', input), { reply_to_message_id: msg.message_id });
            break;
        case '/imagine':
            (async () => {
                while (!done) {
            ...
            try {
                ...
                await bot.sendMessage(msg.chat.id, TRANSLATIONS[userConfig.language || PARAMETERS.LANGUAGE].errors['image-safety'], { reply_to_message_id: msg.message_id });
                done = true;
            }
            break;
        default:
